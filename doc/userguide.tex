...
Using the fitted response function of the system it is then possible to determin a correction curve
...
This set of commands should run without producing any error messages (such as failure to import some modules).
If missing modules are reported it will be necessary to add these and run the test script again.
...
Extra terms also can be added to the normally linear dependence of the detector response to the photon energy;
however this polynomial is not constrained to be positive and the fit may fail.
Hence the vary value defaults to -1 which indicates no energy dependence and setting a value greater than 0 is not recommended.
Note that there is no line dependence in the energy, even for values above 0; this just allow higher order energy terms.
Another experimental option that has now been added is called ``spectra'', which also defaults to -1, i.e. not used.
...
This feature is only for advanced users and may not give good results.
